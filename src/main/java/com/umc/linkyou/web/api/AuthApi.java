...
public interface AuthApi {
    ...
    @Operation(
            ...
            description =
                    """
                    이메일 회원가입을 진행합니다.
                    - 이메일, 닉네임, 비밀번호, 성별, 직업, 사용 목적, 관심사, deviceId, deviceType을 입력받습니다.
                    - 회원가입이 완료되면 자동으로 로그인 처리되어, 사용자 정보와 생성 시각, 액세스/리프레시 토큰을 함께 반환합니다.
                    - `deviceId`/`deviceType`은 로그인 API와 동일한 규칙을 따릅니다(디바이스 고유 식별자, 최대 동시 로그인 디바이스 수 제한에도 동일하게 적용).
    ...
    @ApiErrorCode(
            userErrorStatus = {
                UserErrorStatus._DUPLICATE_JOIN_REQUEST,
                UserErrorStatus._DUPLICATE_NICKNAME
            })
    @PostMapping("/signup")
    ApiResponse<UserResponseDTO.JoinResultDTO> join(
    ...
    @ApiErrorCode(
            userErrorStatus = {
                UserErrorStatus._LOGIN_FAILED,
                UserErrorStatus._USER_INACTIVE,
                UserErrorStatus._SOCIAL_ACCOUNT_ONLY,
                UserErrorStatus._USER_NOT_FOUND
            })
    @PostMapping("/login")
    ApiResponse<UserResponseDTO.LoginResultDTO> login(
    ...
    ApiResponse<UserResponseDTO.TokenPair> reissueToken(
    ...
    @Operation(
            ...
            description =
                    """
                    현재 로그인된 사용자의 닉네임을 조회합니다.
                    - Authorization Bearer 헤더의 Access Token으로 사용자를 식별합니다.
                    - 응답으로 `nickname` 필드 하나만 반환합니다.
                    - 토큰이 없거나 유효하지 않으면 인증 실패 예외를 반환합니다.
                    - 토큰의 사용자가 존재하지 않으면 사용자 없음 예외를 반환합니다.
                    """)
    @ApiAuthSuccessCode(AuthSuccessStatus.NICKNAME_INQUIRY_SUCCESS)
    @ApiErrorCode(authErrorStatus = {AuthErrorStatus.UNAUTHORIZED})
    ...
    @Operation(
            ...
            description =
                    ...
                    - 가입되지 않았거나 활성(ACTIVE) 상태가 아닌(임시/탈퇴) 계정이면 사용자 없음 에러를 반환합니다.
                    - 일반 계정 없이 소셜 로그인으로만 가입된 이메일이면 가입된 소셜 종류(카카오 / 구글 / 카카오+구글)에 따라 소셜 로그인 안내 에러를 반환합니다.
                    - 일반 계정과 소셜 계정이 함께 있는 이메일이면 재설정 링크가 전송됩니다.
                    - 유효하지 않은 이메일 주소이거나 메일 전송에 실패하면 예외를 반환합니다.
                    """)
    @ApiAuthSuccessCode(AuthSuccessStatus.PASSWORD_RESET_LINK_SENT)
    @ApiErrorCode(
            userErrorStatus = {
                UserErrorStatus._INVALID_EMAIL_ADDRESS,
                UserErrorStatus._SEND_MAIL_FAILED,
                UserErrorStatus._USER_NOT_FOUND,
                UserErrorStatus._KAKAO_SOCIAL_ACCOUNT_ALREADY_EXISTS,
                UserErrorStatus._GOOGLE_SOCIAL_ACCOUNT_ALREADY_EXISTS,
                UserErrorStatus._KAKAO_GOOGLE_SOCIAL_ACCOUNT_ALREADY_EXISTS
            })
    @ApiErrorCode(commonErrorStatus = {CommonErrorStatus._TOO_MANY_REQUESTS})
    @PostMapping("/password/reset/send")
    ApiResponse<Object> sendPasswordResetLink(
