...
INSERT INTO fcolors
(fcolor_id, color_name, color_code_1, color_code_2, color_code_3, color_code_4)
VALUES
    (1, '레드', '#FF5353', '#FFA0A0', '#FFC2C2', '#FFEEEE')
ON CONFLICT (fcolor_id) DO UPDATE SET
                     color_name = EXCLUDED.color_name,
                     color_code_1 = EXCLUDED.color_code_1,
                     color_code_2 = EXCLUDED.color_code_2,
                     color_code_3 = EXCLUDED.color_code_3,
                     color_code_4 = EXCLUDED.color_code_4;
...
INSERT INTO categories (category_id, fcolor_id, category_name)
VALUES (1, 1, '어학')
ON CONFLICT (category_id) DO UPDATE SET category_name = EXCLUDED.category_name,
                        fcolor_id = EXCLUDED.fcolor_id;
INSERT INTO categories (category_id, fcolor_id, category_name)
VALUES (2, 2, '뉴스')
ON CONFLICT (category_id) DO UPDATE SET category_name = EXCLUDED.category_name,
                        fcolor_id = EXCLUDED.fcolor_id;
...
INSERT INTO emotions (emotion_id, name)
VALUES (1, '즐거움')
ON CONFLICT (emotion_id) DO UPDATE SET name = EXCLUDED.name;
INSERT INTO emotions (emotion_id, name)
VALUES (2, '평온')
ON CONFLICT (emotion_id) DO UPDATE SET name = EXCLUDED.name;
INSERT INTO emotions (emotion_id, name)
...
-- 즐거움 (emotion_id = 1)
INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 1, '(닉네임)님, 요즘 기분 좋은 일이 많았죠? 그 흐름을 더 이어갈 콘텐츠들이에요.', '지금의 긍정적인 에너지를 놓치지 말고, 조금 더 넓혀봐요!'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 1 AND header_text = '(닉네임)님, 요즘 기분 좋은 일이 많았죠? 그 흐름을 더 이어갈 콘텐츠들이에요.' AND footer_text = '지금의 긍정적인 에너지를 놓치지 말고, 조금 더 넓혀봐요!'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 1, '(닉네임)님, 기분 좋을 때 찾는 콘텐츠는 기억에 오래 남아요. 오늘도 그런 하루가 되기를!', '마음이 열려 있을 때, 생각보다 더 많은 걸 받아들일 수 있거든요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 1 AND header_text = '(닉네임)님, 기분 좋을 때 찾는 콘텐츠는 기억에 오래 남아요. 오늘도 그런 하루가 되기를!' AND footer_text = '마음이 열려 있을 때, 생각보다 더 많은 걸 받아들일 수 있거든요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 1, '(닉네임)님의 좋은 하루, 더 좋은 생각으로 채워볼까요? 지금 이 기분을 조금 더 확장해줄 콘텐츠들이에요.', '행복은 종종 아주 사소한 클릭 하나에서 시작되니까요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 1 AND header_text = '(닉네임)님의 좋은 하루, 더 좋은 생각으로 채워볼까요? 지금 이 기분을 조금 더 확장해줄 콘텐츠들이에요.' AND footer_text = '행복은 종종 아주 사소한 클릭 하나에서 시작되니까요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 1, '(닉네임)님, 기분 좋은 지금, 뭔가 새롭고 흥미로운 걸 만나보는 건 어때요?', '즐거움은 나누면 배가 되죠. 이 감정을 오래 기억할 수 있기를!'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 1 AND header_text = '(닉네임)님, 기분 좋은 지금, 뭔가 새롭고 흥미로운 걸 만나보는 건 어때요?' AND footer_text = '즐거움은 나누면 배가 되죠. 이 감정을 오래 기억할 수 있기를!'
);

-- 평온 (emotion_id = 2)
INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 2, '(닉네임)님, 딱히 무슨 감정이 있는 건 아니지만, 그럴수록 이런 콘텐츠가 필요한 순간일지도 몰라요.', '의미 없는 것들 속에서 가끔 중요한 게 튀어나오기도 해요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 2 AND header_text = '(닉네임)님, 딱히 무슨 감정이 있는 건 아니지만, 그럴수록 이런 콘텐츠가 필요한 순간일지도 몰라요.' AND footer_text = '의미 없는 것들 속에서 가끔 중요한 게 튀어나오기도 해요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 2, '(닉네임)님, 그냥 그런 하루였죠. 아무 생각 없이 클릭해도 괜찮아요.', '가끔은 아무 의도 없이 마주한 것들이, 의외로 오래 남아요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 2 AND header_text = '(닉네임)님, 그냥 그런 하루였죠. 아무 생각 없이 클릭해도 괜찮아요.' AND footer_text = '가끔은 아무 의도 없이 마주한 것들이, 의외로 오래 남아요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 2, '(닉네임)님, 감정의 여백이 많은 하루, 그 틈을 살짝 채워줄 콘텐츠예요.', '오늘은 아무 기대 없이 읽어도 좋아요. 그런 날도 필요한 법이니까요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 2 AND header_text = '(닉네임)님, 감정의 여백이 많은 하루, 그 틈을 살짝 채워줄 콘텐츠예요.' AND footer_text = '오늘은 아무 기대 없이 읽어도 좋아요. 그런 날도 필요한 법이니까요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 2, '(닉네임)님, 할 일은 많은데 마음은 비어있을 때, 그냥 스쳐볼 수 있는 콘텐츠가 필요하잖아요.', '모두가 특별할 필요는 없어요. 그냥 그런 오늘도 충분히 의미 있어요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 2 AND header_text = '(닉네임)님, 할 일은 많은데 마음은 비어있을 때, 그냥 스쳐볼 수 있는 콘텐츠가 필요하잖아요.' AND footer_text = '모두가 특별할 필요는 없어요. 그냥 그런 오늘도 충분히 의미 있어요.'
);

-- 설렘 (emotion_id = 3)
INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 3, '(닉네임)님, 설레는 기분이 있다면, 뭔가가 시작될 준비가 된 거예요.', '지금의 감정이 어디로 가게 될지, 조금 더 지켜봐도 좋겠어요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 3 AND header_text = '(닉네임)님, 설레는 기분이 있다면, 뭔가가 시작될 준비가 된 거예요.' AND footer_text = '지금의 감정이 어디로 가게 될지, 조금 더 지켜봐도 좋겠어요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 3, '(닉네임)님의 마음이 조금씩 움직이는 것 같아요. 그 떨림에 어울리는 콘텐츠를 골라봤어요.', '설렘은 가장 강력한 동기부여예요. 지금, 그 에너지를 믿어보세요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 3 AND header_text = '(닉네임)님의 마음이 조금씩 움직이는 것 같아요. 그 떨림에 어울리는 콘텐츠를 골라봤어요.' AND footer_text = '설렘은 가장 강력한 동기부여예요. 지금, 그 에너지를 믿어보세요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 3, '(닉네임)님, 기대하는 일이 있을 때, 마음이 조금 더 섬세해지죠.', '설렘을 오래 붙잡지 않아도 돼요. 잠시 머물러도 충분하니까요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 3 AND header_text = '(닉네임)님, 기대하는 일이 있을 때, 마음이 조금 더 섬세해지죠.' AND footer_text = '설렘을 오래 붙잡지 않아도 돼요. 잠시 머물러도 충분하니까요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 3, '(닉네임)님, 어떤 일이든 잘 풀릴 것 같은 예감이 드는 날엔 이런 콘텐츠가 의외로 잘 맞아요.', '좋은 에너지는 늘 다음을 향하고 있어요. 지금도 그중 하나일 거예요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 3 AND header_text = '(닉네임)님, 어떤 일이든 잘 풀릴 것 같은 예감이 드는 날엔 이런 콘텐츠가 의외로 잘 맞아요.' AND footer_text = '좋은 에너지는 늘 다음을 향하고 있어요. 지금도 그중 하나일 거예요.'
);

-- 슬픔 (emotion_id = 4)
INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 4, '(닉네임)님, 요즘 마음이 자주 무거웠죠. 한 걸음 멈춰, 지금의 나에게 필요한 것들을 함께 살펴봐요.', '계속 괜찮은 척 안 해도 돼요. 마음이 머무를 곳, 여기 있어요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 4 AND header_text = '(닉네임)님, 요즘 마음이 자주 무거웠죠. 한 걸음 멈춰, 지금의 나에게 필요한 것들을 함께 살펴봐요.' AND footer_text = '계속 괜찮은 척 안 해도 돼요. 마음이 머무를 곳, 여기 있어요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 4, '(닉네임)님, 혼자만 복잡한 게 아니에요. 비슷한 고민을 지나온 이들의 이야기로 당신의 오늘을 밝혀줄게요.', '지금 이 순간, 아무것도 하지 않아도 괜찮아요. 우린 늘 다시 일어설 힘을 가지고 있으니까요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 4 AND header_text = '(닉네임)님, 혼자만 복잡한 게 아니에요. 비슷한 고민을 지나온 이들의 이야기로 당신의 오늘을 밝혀줄게요.' AND footer_text = '지금 이 순간, 아무것도 하지 않아도 괜찮아요. 우린 늘 다시 일어설 힘을 가지고 있으니까요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 4, '생각은 많은데 정리가 안 되죠. (닉네임)님의 머릿속을 환기시켜줄 콘텐츠들을 모았어요', '지금 떠오르지 않아도 괜찮아요. 영감은 가끔, 쉬고 있을 때 더 잘 찾아오거든요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 4 AND header_text = '생각은 많은데 정리가 안 되죠. (닉네임)님의 머릿속을 환기시켜줄 콘텐츠들을 모았어요' AND footer_text = '지금 떠오르지 않아도 괜찮아요. 영감은 가끔, 쉬고 있을 때 더 잘 찾아오거든요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 4, '(닉네임)님, 마음이 무거운 날엔, 다독임보다 방향이 필요한 걸지도 몰라요.', '지금은 가만히 있어도 돼요. 그 자체로도 충분히 잘하고 있는 거니까요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 4 AND header_text = '(닉네임)님, 마음이 무거운 날엔, 다독임보다 방향이 필요한 걸지도 몰라요.' AND footer_text = '지금은 가만히 있어도 돼요. 그 자체로도 충분히 잘하고 있는 거니까요.'
);

-- 짜증 (emotion_id = 5)
INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 5, '(닉네임)님, 요즘 작은 일에도 쉽게 예민해지죠. 그럴 땐 잠깐 다른 결의 콘텐츠를 보는 것도 괜찮아요.', '짜증을 억누르지 않아도 돼요. 단지, 그 에너지를 흘릴 통로만 있으면 돼요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 5 AND header_text = '(닉네임)님, 요즘 작은 일에도 쉽게 예민해지죠. 그럴 땐 잠깐 다른 결의 콘텐츠를 보는 것도 괜찮아요.' AND footer_text = '짜증을 억누르지 않아도 돼요. 단지, 그 에너지를 흘릴 통로만 있으면 돼요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 5, '(닉네임)님의 답답한 기분, 괜찮아요. 이 콘텐츠들이 잠시 머리를 환기시켜줄 수 있을 거예요.”', '감정은 느끼는 대로 두세요. 똑똑한 선택은 가끔 쉬는 것부터 시작돼요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 5 AND header_text = '(닉네임)님의 답답한 기분, 괜찮아요. 이 콘텐츠들이 잠시 머리를 환기시켜줄 수 있을 거예요.”' AND footer_text = '감정은 느끼는 대로 두세요. 똑똑한 선택은 가끔 쉬는 것부터 시작돼요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 5, '(닉네임)님, 짜증이 쌓인 날엔 의미 없는 정보도 거슬리죠. 그래서 가볍고 유연한 콘텐츠들로 준비했어요.', '감정이 무뎌지기 전, 살짝 숨 고르듯 흘려보면 좋아요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 5 AND header_text = '(닉네임)님, 짜증이 쌓인 날엔 의미 없는 정보도 거슬리죠. 그래서 가볍고 유연한 콘텐츠들로 준비했어요.' AND footer_text = '감정이 무뎌지기 전, 살짝 숨 고르듯 흘려보면 좋아요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 5, '(닉네임)님, 계속해서 끓어오르기 전에, 지금 멈춰서 다른 쪽을 바라보는 것도 하나의 전략이에요.', '감정의 방향을 바꾸기보다, 감정이 흐를 수 있게 해주세요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 5 AND header_text = '(닉네임)님, 계속해서 끓어오르기 전에, 지금 멈춰서 다른 쪽을 바라보는 것도 하나의 전략이에요.' AND footer_text = '감정의 방향을 바꾸기보다, 감정이 흐를 수 있게 해주세요.'
);

-- 분노 (emotion_id = 6)
INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 6, '(닉네임)님, 화가 날 땐 말보다 생각이 많아지죠. 그 감정을 다루기 위한 콘텐츠들을 모았어요.', '감정은 정리하지 않아도 괜찮아요. 다만, 흘릴 구멍은 있어야 하니까요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 6 AND header_text = '(닉네임)님, 화가 날 땐 말보다 생각이 많아지죠. 그 감정을 다루기 위한 콘텐츠들을 모았어요.' AND footer_text = '감정은 정리하지 않아도 괜찮아요. 다만, 흘릴 구멍은 있어야 하니까요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 6, '(닉네임)님, 억울하고 화가 날 땐, 세상이 너무 불공평하게 느껴지죠. 그럴 때 필요한 건 날카로운 인사이트일지도 몰라요.', '이유 없는 감정은 없어요. 지금 이 감정도 당신의 한 부분이에요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 6 AND header_text = '(닉네임)님, 억울하고 화가 날 땐, 세상이 너무 불공평하게 느껴지죠. 그럴 때 필요한 건 날카로운 인사이트일지도 몰라요.' AND footer_text = '이유 없는 감정은 없어요. 지금 이 감정도 당신의 한 부분이에요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 6, '(닉네임)님, 분노는 무기력이 아니라 에너지예요. 어떻게 쓸지, 지금 여기서 천천히 살펴볼 수 있어요.', '당장 해결은 안 되더라도, 그 감정을 외면하지 않아줘서 고마워요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 6 AND header_text = '(닉네임)님, 분노는 무기력이 아니라 에너지예요. 어떻게 쓸지, 지금 여기서 천천히 살펴볼 수 있어요.' AND footer_text = '당장 해결은 안 되더라도, 그 감정을 외면하지 않아줘서 고마워요.'
);

INSERT INTO curation_ments (emotion_id, header_text, footer_text)
SELECT 6, '(닉네임)님의 감정이 폭발하기 전에 한 번 숨 고를 수 있도록 도와줄 콘텐츠들이에요.', '모든 감정은 지나가요. 오늘의 분노도 결국 당신을 더 단단하게 만들 거예요.'
WHERE NOT EXISTS (
    SELECT 1 FROM curation_ments
    WHERE emotion_id = 6 AND header_text = '(닉네임)님의 감정이 폭발하기 전에 한 번 숨 고를 수 있도록 도와줄 콘텐츠들이에요.' AND footer_text = '모든 감정은 지나가요. 오늘의 분노도 결국 당신을 더 단단하게 만들 거예요.'
);
...
INSERT INTO situations (situation_id, name) VALUES
-- 고등학생 (1~8)
(1, '통학 중'),
(2, '공부 중'),
(3, '식사 중'),
...
(47, '휴식 중'),
(48, '자기 전')
    ON CONFLICT (situation_id) DO UPDATE SET name = EXCLUDED.name;


INSERT INTO situation_jobs (situation_job_id, situation_id, job_id) VALUES
...
INSERT INTO situation_categories (situation_category_id, situation_id, category_id) VALUES
      (1, 1, 1), (2, 1, 14), (3, 1, 11),
      (4, 2, 3), (5, 2, 5), (6, 2, 10),
      (7, 3, 14), (8, 3, 15),
...
SELECT setval(pg_get_serial_sequence('fcolors', 'fcolor_id'), COALESCE((SELECT MAX(fcolor_id) FROM fcolors), 1), true);
SELECT setval(pg_get_serial_sequence('categories', 'category_id'), COALESCE((SELECT MAX(category_id) FROM categories), 1), true);
SELECT setval(pg_get_serial_sequence('emotions', 'emotion_id'), COALESCE((SELECT MAX(emotion_id) FROM emotions), 1), true);
SELECT setval(pg_get_serial_sequence('curation_ments', 'curation_ment_id'), COALESCE((SELECT MAX(curation_ment_id) FROM curation_ments), 1), true);
SELECT setval(pg_get_serial_sequence('jobs', 'job_id'), COALESCE((SELECT MAX(job_id) FROM jobs), 1), true);
SELECT setval(pg_get_serial_sequence('situations', 'situation_id'), COALESCE((SELECT MAX(situation_id) FROM situations), 1), true);
SELECT setval(pg_get_serial_sequence('situation_jobs', 'situation_job_id'), COALESCE((SELECT MAX(situation_job_id) FROM situation_jobs), 1), true);
SELECT setval(pg_get_serial_sequence('situation_categories', 'situation_category_id'), COALESCE((SELECT MAX(situation_category_id) FROM situation_categories), 1), true);
